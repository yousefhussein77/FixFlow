...
class AdminTicketCommentController extends Controller
{
    ...
    {
        try {
            $comments = $action->execute($request->user(), User::ROLE_ADMINISTRATOR, $reference);
    ...
    {
        try {
            $result = $action->execute($request->user(), User::ROLE_ADMINISTRATOR, $reference, $request->string('content')->toString(), $request->string('submission_token')->toString());
    ...
    {
        TicketEvent::record('ticket.comment_target', 'not_found', $request->user()->id);
