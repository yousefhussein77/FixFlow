...
class ReporterTicketCommentController extends Controller
{
    public function index(Request $request, string $reference, ListTicketComments $action): JsonResponse
    {
        try {
            $comments = $action->execute($request->user(), User::ROLE_REPORTER, $reference);
    ...
    {
        try {
            $result = $action->execute($request->user(), User::ROLE_REPORTER, $reference, $request->string('content')->toString(), $request->string('submission_token')->toString());
